...
export async function handleHelp(i: ChatInputCommandInteraction) {
  await i.reply({
    ...
    content: [
      ...
      "• `/share <bet_id>` — get a prefilled tweet (post + `/confirm-share` to drop your fee from 250→150bps; first-time only: `/linktwitter <handle>`)",
      "",
      "**Info commands**",
      ...
      "• Admins claim cases with `/arbiter-claim`, then DM both sides for evidence, review with `/arbiter-review`, and finalize with `/arbiter-decide @winner`. `/admin-stats` shows the bot's health snapshot.",
      "",
